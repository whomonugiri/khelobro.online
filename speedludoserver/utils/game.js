...
export const fetchGameData = async ({ gameUid, token, deviceId }) => {
  try {
    ...
    const game = await SpeedLudo.findOne({
      _id: gameUid,
      status: "running",
...
export const updateGameData = async ({ gameUid, data }) => {
  try {
    const game = await SpeedLudo.findOne({ _id: gameUid }).lean();
    ...
    if (game && game.status == "running") {
      ...
      if (data.winScore == data.loseScore && !data.sc) {
        ...
        const NewTxn = {
          ...
          amount: game.prize,
          cash: bet.cash,
          reward: game.prize - game.entryFee + Number(bet.reward),
          bonus: bet.bonus,
          remark: "Match Won",
          status: "completed",
          txnType: "credit",
